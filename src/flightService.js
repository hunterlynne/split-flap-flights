...
export function getFlightsMock(prev = []) {
  // ARRIVALS (IAD) — seed on first call
...
export function getDeparturesMock(prev = []) {
  // DEPARTURES (IAD) — seed on first call
  if (!prev || prev.length === 0) {
    return [
      {
        flight: "UA927",
...
function randomDrift(list) {
  return list.map((f) => {
    ...
    if (roll < 0.25) {
      ...
      const gates = ["A1", "B4", "B12", "C3", "C9", "D7", "D11", "E5"];
...
function driftTime(hhmm) {
  ...
  const offset = Math.floor(Math.random() * 11) - 5; // -5..+5 minutes
