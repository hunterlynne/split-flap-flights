...
import { getFlightsMock, getDeparturesMock } from "./flightService";

const PAGE_SIZE = 5; // change to 8, 10, etc.
const ROTATE_MS = 7000; // auto-rotate every 7s
...
export default function App() {
  ...
  useEffect(() => {
    setFlights(
      mode === "arrivals" ? getFlightsMock([]) : getDeparturesMock([])
    );
    setPage(0); // reset to first page when switching mode
  }, [mode]);
  ...
  // ----- Auto-fit board scaling + tile measurement -----
  const containerRef = useRef(null);
  const innerRef = useRef(null);
  const [scale, setScale] = useState(1);

  // Measure tile width (including margins) and expose as --tilew for CSS columns
  useLayoutEffect(() => {
    const measureTile = () => {
  ...
  // Fit the board to available width (uses transform scale)
  useLayoutEffect(() => {
  ...
  }, [visible.length, page, mode]); // recalc when page changes
  ...
  return (
    <div className="App" style={{ padding: 24 }}>
      ...
      <div className="toolbar">
        <h1>
          {mode === "arrivals" ? "Arrivals" : "Departures"} — DULLES (IAD)
        </h1>
        <div className="controls">
          <button
            onClick={() => setMode("arrivals")}
            disabled={mode === "arrivals"}
          >
            Arrivals
          </button>
          <button
            onClick={() => setMode("departures")}
            disabled={mode === "departures"}
          >
            Departures
          </button>
      ...
      <div className="headers">
        ...
        <div className="col-origin">
          {mode === "arrivals" ? "ORIGIN" : "DESTINATION"}
        </div>
